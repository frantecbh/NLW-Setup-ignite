  ...
  return (
    <View className="flex-1 bg-background px-8 pt-16">
      ...
      >
        ...
        <TextInput 
        placeholder="ex.:Exercitar, dormir bem..." 
        placeholderTextColor={colors.zinc[400]} 
        className='h-12 pl-4 mt-3 bg-zinc-900 text-white rounded-lg border-2 border-zinc-800 focus:border-green-600'
        ...
        <Text className='font-semibold mt-4 mb-3 text-white text-base' >Qual a recorrência? </Text>
        {
          availableWeekDays.map((weekDay, index) => {
            return (
              <CheckBox key={weekDay} checked={weekDays.includes(index)} texto={weekDay} onPress={() => handleToggleWeekDay(index)} />
            )
          })
        }
